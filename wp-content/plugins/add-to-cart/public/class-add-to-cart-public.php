...
class Add_To_Cart_Public {
	...
	public function ced_create_page_for_single_post($single_page_temp) {
		...
		if(is_singular($post_type)) {
			$single_page_temp = plugin_dir_path( __FILE__ ) . 'partials/add-to-cart-singletemp.php';
		}
	...
	public function ced_create_page_for_add_to_cart($template){
		...
		if($page_name == 'ced_addtocart_page'){
			$template = plugin_dir_path(__FILE__). 'partials/add-to-cart-addtocart.php';
		} 
		return $template;
	}
	/**
	...
	public function generate_short_code_for_page($ret) {
		require plugin_dir_path( __FILE__ ) . 'partials/add-to-cart-page-data.php';
	 return ;
	}
}
